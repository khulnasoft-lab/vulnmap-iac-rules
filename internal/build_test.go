...
func TestBuildWithRuleIdsWithVulnmapPrefix(t *testing.T) {
	baseTestFiles := map[string]string{
		"test1.rego": `
			package test
			msg = {
				"publicId": "VULNMAP-TEST-1",
				"severity": "low"
			}
		`,
	}
	...
	test.WithTempFS(baseTestFiles, func(root string) {
		...
		err = RunBuild([]string{root}, buildParams)
		assert.NotNil(t, err)
		assert.Contains(t, err.Error(), "Custom rules cannot have a name that starts with \"VULNMAP-\"")
		assert.Contains(t, err.Error(), fmt.Sprintf("%s/test1.rego", root))
	})
}
