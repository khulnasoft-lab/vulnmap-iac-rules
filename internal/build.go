...
func assertRuleIdsWithoutVulnmapPrefix(rules []util.Rule) error {
	invalidRulesPaths := []string{}

	for _, rule := range rules {
		if strings.HasPrefix(rule.PublicId, "VULNMAP-") {
			invalidRulesPaths = append(invalidRulesPaths, rule.Path)
		}
	}
	...
	if len(invalidRulesPaths) > 0 {
		errMessage := "We cannot create a bundle for your custom rules." +
			"\nCustom rules cannot have a name that starts with \"VULNMAP-\"." +
			"\nPlease ensure your public ID does not start with \"VULNMAP-\"." +
			"\n\nRules that start with \"VULNMAP-\" are:"
		for _, invalidRulePath := range invalidRulesPaths {
			errMessage += fmt.Sprintf("\n- %s", invalidRulePath)
		}
		return errors.New(errMessage)
...
func assertValidRules(paths []string) error {
	...
	err = assertRuleIdsWithoutVulnmapPrefix(rules)
	if err != nil {
		return err
	}
